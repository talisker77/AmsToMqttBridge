...
namespace HanDebugger
{
    class Program
    {
        ...
        private static void WriteAndEmptyBuffer()
        {
            ...
            System.Console.WriteLine("Current consumption: {0:D3}kW", consume / 10000);
            consumption.Add(consume);
            System.Console.WriteLine("Average consumption: {0:D3}kW", consumption.Average() / 10000);

            if (listId == HanDebugger.KaifaHanBeta.List3)
            {
                var start = 1073;
                var currentAnualConsumption = KaifaHanBeta.GetInt(start, line, 0, line.Length);
                System.Console.WriteLine("Current anual consumption is: {0:D3}kW/h", currentAnualConsumption / 10000);
            }
